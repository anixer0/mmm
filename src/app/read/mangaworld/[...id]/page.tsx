import { fetchMangaWorldInfo } from "@/app/components/request";
import ChapterSelector from "@/app/components/chapterSelection";

import Image from "next/image";

const MangaworldInfoPage = async ({ params }: { params: { id: string[] } }) => {
	const joinedString = params.id.join("/");
	const data = await fetchMangaWorldInfo(joinedString);
	return (
		<div className="p-2 lg:w-7/12 m-auto">
			...
			<div role="tablist" className="tabs tabs-lifted tabs-lg mt-1">
				...
				>
					...
					<p>Genres: {data.results.genres}</p>
				</div>
				...
				>
					<label className="form-control w-full">
						...
						<ChapterSelector data={data} provider="mangaworld" />
					</label>
				</div>
			</div>
		</div>
...
export default MangaworldInfoPage;
